
commands["directory"] = function(args){
  window.open('https://canadaecoin.directory', 'indieexpress'); 
  return `opened indie express via example 'canadaecoin.directory' in browser window`;
};

commands["disco"] = function(args){
  window.open('https://player.koad.sh', 'koadplayer'); 
  return `opened 'player.koad.sh' in browser window`;
};

commands["discord"] = function(args){
  window.open('https://discord.gg/Qnhwtqmcb2', '_blank'); 
  return `opened invite to koad's discord channel in browser window `;
};

commands["docs"] = function(args){
  window.open('https://book.koad.sh', 'koadiodocs'); 
  return `opened 'book.koad.sh' in browser window`;
};

commands["github"] = function(args){
  window.open('https://github.com/koad', 'github'); 
  return `opened 'github.com/koad' in browser window`;
};

commands["keybase"] = function(args){
  window.open('https://keybase.io/koad', 'koadlive'); 
  return `opened 'keybase.io/koad' in browser window`;
};

commands["live"] = function(args){
  window.open('https://live.koad.sh', 'koadlive'); 
  return `opened 'live.koad.sh' in browser window`;
};

commands["matrix"] = function(args){
  window.open('https://matrix.to/#/#live:koad.sh', '_blank'); 
  return `opened invite to koad's live matrix space in browser window `;
};

commands["scan"] = function(args){
  window.open('https://scan.koad.sh', 'scanner'); 
  return `opened 'scan.koad.sh' in browser window`;
};

commands["wallet"] = function(args){
  window.open('https://ecoincore.com', 'ecoincore'); 
  return `opened 'ecoincore.com' in browser window`;
};
